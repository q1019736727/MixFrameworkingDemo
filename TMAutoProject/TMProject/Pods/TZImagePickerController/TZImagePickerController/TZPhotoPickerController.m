...
- (void)initSubviews {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self->_models && tzImagePickerVc.videoMaximumDuration > 0.01) {
            NSMutableArray *deleteArray = [NSMutableArray array];
            for (TZAssetModel *model in self->_models) {
                
                if (model.type == TZAssetCellTypeVideo && model.asset.duration > tzImagePickerVc.videoMaximumDuration) {
                    [deleteArray addObject:model];
                }
            }
            [self->_models removeObjectsInArray:deleteArray];
        }
